...
async function outputCss(file) {
  ...
  try {
    const precss = await compileScss(file)
    const css = await compilePostCss(precss, file, output)
    await mkdir(output)
    await writeFile(output, css)
    console.log(
      colors.green(" compile scss file: ") + path.relative(rootPath, output)
    )
  } catch (err) {
    console.error(err)
    throw err
...
async function watch(input) {
  ...
  const emittyScss = emitty.setup(config.inputDir, "scss", {
    basedir: config.inputDir
  })
  await emittyScss.scan()
  ...
  watcher.on("all", async (event, file) => {
    if (event === "add" || event === "unlink") {
      await emittyScss.scan().catch(err => {
        throw new Error(err)
      })
    }
    if (new RegExp(`${path.sep}_`).test(file) === false) {
      ...
      const storage = emittyScss.storage()
